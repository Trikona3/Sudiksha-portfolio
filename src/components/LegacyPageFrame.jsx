...
import { transformLegacyHtml } from '../utils/transformLegacyHtml';
...
function LegacyPageFrame({ html, title }) {
  ...
  useLayoutEffect(() => {
    const isHomeTop = !location.hash || location.hash === '#home';
    if (isHomeTop) {
      window.scrollTo(0, 0);
  ...
  useEffect(() => {
    ...
    const headAssetSelector = 'link[rel="stylesheet"], style';
    doc.head.querySelectorAll(headAssetSelector).forEach((node) => {
    ...
    const onClick = (event) => {
      ...
      // Let browser handle file downloads and new-tab links.
      ...
      // Static assets (pdf/images/videos/docs) should not be routed by React.
    ...
    const runScripts = async () => {
        ...
        for (const sourceScript of scripts) {
          ...
          if (src) {
            try {
              ...
              // Keep rendering the rest of the page even if one script fails.
            }
            continue;
          }
          ...
          const scriptBody = sourceScript.textContent || '';
          // Wrap legacy inline scripts to avoid global const/let redeclaration
          // when users navigate away and return to the same page.
      ...
      pendingLoadCallbacks.forEach((callback) => {
        try {
          ...
          // Ignore callback failures to keep page render resilient.
        }
      });

      await ensureGlobalAosLoaded().catch(() => {});

      // Let #process keep AOS behavior; all other elements use the custom
      // reveal engine so left/right directions are consistent everywhere.
      ...
      const initAutoReveal = () => {
        ...
        const targets = uniqueNodes.filter((node) => {
          ...
          if (node.closest('#process')) {
            return false;
          }
          if (node.closest('header, nav, footer')) {
            return false;
          }
        ...
        targets.forEach((node, index) => {
          ...
          if (node.matches('img, video, figure')) {
  ...
  }, [location.hash, navigate, title, transformedHtml]);
